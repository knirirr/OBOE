	 ...
	 function Pen(map){
		...
		this.drawPolygon = function (listOfDots,color,des,id){
			...
			this.deleteMis();
      /* the following code updates a page element and therefore
       * shouldn't really be here, but putting it here does make
       * the element update occur immediately that the polygon is
       * closed, which is much nicer for the user than having to
       * make them click on a button
       * ...banana...
       */
       var existing = $('#job_parameters_coords').html();
       if (existing === '')
       {
         $('#job_parameters_coords').append(this.getData());
       }
       else
       {
         var newinfo =  existing + ';' + this.getData();
         $('#job_parameters_coords').html(newinfo);
       }
		}
	 ...
	 function Info(polygon,map){
		...
		this.changeColor= function(){
			thisObj.parent.setColor($(thisObj.color).val());
		}
