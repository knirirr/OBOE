function bict(argsfile)
% BICT  Compile target for BICT jobs on the OBOE automation server.
%
%   BICT inputfile
%
% Compile options:
%   mcc -m -N -p map -p stats -p images bict.m
%
...
try
    ...
    logmsg(ME,'Can''t delete template files')
end
clear whichtemplate
% ------------------------------------------------------------------------
% Write output.pdf
...
for i = find(ismember(devflags,fields))'
    switch data.args.(devflags{i})
        case 'yes'
            data.devflags.(devflags{i}) = 1;
        case 'no'
            data.devflags.(devflags{i}) = 0;
    end
end
% ------------------------------------------------------------------------
...
% Special task for BICT
% Move/Copy <jobid>.in.txt to indices.txt
file = fullfile(work,'input','data',sprintf('%s.in.txt',data.job.id));
if exist(file,'file')
    if isdeployed
        movefile(file,fullfile(work,'output','data','indices.txt'))
    else
        copyfile(file,fullfile(work,'output','data','indices.txt'))
    end
else
    logmsg(1,'%s file not found',file)
end

% Move/Copy <jobid>.in.csv to indices.csv
file = fullfile(work,'input','data',sprintf('%s.in.csv',data.job.id));
if exist(file,'file')
    if isdeployed
        movefile(file,fullfile(work,'output','data','indices.csv'))
    else
        copyfile(file,fullfile(work,'output','data','indices.csv'))
    end
else
    logmsg(1,'%s file not found',file)
end
